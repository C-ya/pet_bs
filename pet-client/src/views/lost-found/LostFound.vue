                ...
                <sui-feed-content>
                  <sui-feed-summary>
                    <sui-label class="lost-found-category" basic
                               :color="lostFound.lostFoundCategory === '寻宠启事'? 'red':'green'">
      ...
      <iCol class="column" span="8">
        ...
        <sui-card class="fluid">
          <sui-message attached="top" icon="eye" negative>
            ...
            <p>什么，宠物走丢啦？不要怕，快来发布一条寻宠启事吧，很快就会有好心人联系你啦。对啦，一定要核实信息，准确后再发布哦 😃</p>
          </sui-message>
          <sui-button attached="bottom" icon="add" content="发布寻宠启事" @click="showLostFoundModal('寻宠启事')"/>
        </sui-card>
        <!-- 寻宠启事提示 END -->
        <!-- 失宠招领提示 -->
        <sui-card class="fluid">
          <sui-message attached="top" icon="bullhorn" positive>
            <sui-message-content>
              ...
              <p>发现一只疑似走丢的小可爱？快发布一条失宠招领吧，失主一定会非常非常感激你呢 😘</p>
            </sui-message-content>
          </sui-message>
          <sui-button attached="bottom" icon="add" content="发布失宠招领" @click="showLostFoundModal('失宠招领')"/>
        </sui-card>
        <!-- 失宠招领提示 END -->
      </iCol>
    </Row>
    ...
    <Modal v-model="lostFoundAddModal.show" :title="lostFoundAddModal.title" width="600" scrollable>
      <Form :model="lostFoundAddModal.form" :label-width="50">
        ...
        <FormItem label="昵称" required>
          <Input v-model="lostFoundAddModal.form.itemName" size="large" placeholder="请填写宠物名"/>
        </FormItem>
        ...
        <FormItem label="描述" required>
          <Input v-model="lostFoundAddModal.form.description" type="textarea" size="large"
                 :autosize="{minRows: 5,maxRows: 10}" placeholder="输入宠物描述，有助于他人获取信息"/>
        </FormItem>
