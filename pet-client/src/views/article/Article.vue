    ...
    <Row class="article-menu">
      <iCol class="article-category" span="18">
        ...
        <template v-for="item in articleCategory">
          <a
            is="sui-label"
            v-if="activeArticleCategory === item.name"
            color="teal"
      ...
      <!-- <iCol span="6">
        <sui-input
          ...
          @keydown.enter="getArticlePage(param)"
        />
      </iCol> -->
    </Row>
    <Row>
            ...
            <sui-feed class="article-feed">
              ...
              >
                ...
                <sui-feed-content>
                  ...
                  <sui-feed-meta>
                    <sui-feed-like @click="addZan(article.resource)">
...
export default {
  ...
  methods: {
    ...
    getArticleCategory() {
      this.$axios
        .get("/api/dic/item", { dicTypeCode: "article-category" })
        .then((res) => {
          let result = res.data;
    ...
    loadMore() {
      this.param.current++;
      this.$axios.get("/api/article/page", this.param).then((res) => {
        let result = res.data;
